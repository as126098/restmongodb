...
public class JSONValidationUtil {
	...
	public static ProcessingReport isJsonValidForReport(JsonSchema jsonSchemaNode, JsonNode jsonNode)
...
*/		return report;
	...
	public static ProcessingReport isJsonValidForReport(File schemaFile, String jsonText) throws ProcessingException, IOException {
		final JsonSchema schemaNode = getSchemaNode(schemaFile);
		final JsonNode jsonNode = getJsonNode(jsonText);
		return isJsonValidForReport(schemaNode, jsonNode);
	} //
	...
	private static JsonSchema _getSchemaNode(JsonNode jsonNode) throws ProcessingException {
		...
		if (null == schemaIdentifier) {
			((ObjectNode) jsonNode).put(JSON_SCHEMA_IDENTIFIER_ELEMENT, JSON_V6_SCHEMA_IDENTIFIER);
		}
